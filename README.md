...
Pengenalan ke Logika Pemrograman (Programming Logic 101). (Progress 18%)
  ...
  * Belajar logika deduktif;
     ...
     - Contoh penggunaan algoritma deduktif;
       ...
       2.Beberapa ular adalah hewan berbisa
       3.Kesimpulannya : beberapa ular adalah hewan berbahaya.
     - Proses penalaran deduktif dimulai dari;
       1.Premis pertama yang berisi pernyataan umum. Jika premis pertama itu valid, seharusnya juga valid untuk segala kasus yang berhubungan dengan premis tersebut.
...
Pengenalan ke Logika Pemrograman (Programming Logic 101). (Progress 100%)
  ...
  * Gerbang Logika Not And
    - Gerbang logika NAND merupakan gabungan dari logika NOT ditambah logika AND yang menghasilkan output yang berlawanan dengan gerbang logika AND. Maka dari itu ia         disebut NAND atau NOT AND. Gerbang logika NAND setidaknya harus memiliki dua masukan untuk mendapatkan satu keluaran. Dalam tabel kebenaran logika NAND di atas         terdapat nilai A = 1 dan B = 1 menghasilkan output X = 0. Mengapa demikian? Karena ini logika NOT AND, maka hasilnya adalah negasi atau kebalikan dari logika           AND. Gerbang logika NAND akan menghasilkan keluaran 0 (false) jika semua masukannya bernilai 1 (true). Sedangkan jika salah satu masukannya ada yang bernilai 0         (false), maka hasil outputnya menjadi 1 (true).
  * Gerbang Logika Not Or
    - Gerbang logika NOR merupakan gabungan dari logika NOT dan logika OR yang menghasilkan output berlawanan dengan gerbang logika OR. Maka dari itu ia disebut NOR         atau NOT OR. Gerbang logika NOR setidaknya harus memiliki dua masukan untuk mendapatkan satu keluaran. Dalam tabel kebenaran logika NOR di atas terdapat nilai A       = 0 dan B = 1 menghasilkan output 0. Mengapa demikian? Kembali lagi ke prinsip logika NOR yang merupakan kebalikan dari OR, jika salah satu input saja ada yang         bernilai 1 (true), maka hasil outputnya adalah 0 (false). Sedangkan jika semua inputnya bernilai 0 (false), maka hasilnya adalah 1 (true).
  * Gerbang Logika Xor
...
Belajar Dasar Pemrograman Javascript
  ...
  * Pengenalan Javascript
    - Javascript merupakan bahasa pemrograman tingkat tinggi untuk membuat website lebih interaktif.
    - Javascript termasuk ke dalam kategori scripting language, artinya kode tidak perlu dikompilasi agar bisa dijalankan. Scripting language menggunakan interpreter         untuk menerjemahkan kode atau perintah yang dimengerti oleh mesin.
    - Javascript termasuk bahasa yang case sensitive, CONSOLE.LOG tidak sama dengan console.log
...
**Kamis, 16 Maret 2023**
Belajar Dasar Pemrograman Javascript
    - bigint, tipe data bisa menyimpan number dengan cakupan nilai yang lebih besar dibandingkan number, untuk membedakan keduanya bisa menambahkan "n" di akhir angka.       tipe data ini juga bisa digunakan untuk nilai yang lebih kecil, bisa juga digunakan untuk operasi aritmatika seperti pembagian, namun bedanya hasilnya akan             dibulatkan kebawah dan tanpa mengandung nilai desimal.
    - strings, tipe data yang bisa menyimpan sebuah teks, untuk menetapkan nilai sebagai string pada variabel gunakan tanda petik satu (') atau petik dua (") di antara       teksnya. jika kita memiliki isi variabel menggunakan tanda petik, supaya tidak error kita bisa menggunakan backslash (\) untuk mengurangi ambiguitas dalam tanda       petik. ini juga dikenal dengan nama escape string. backslash sebelum tanda petik akan memberitahukan interpreter bahwa itu hanyalah tanda petik dan tidak boleh         ditafsirkan sebagai pembatas string. pada string kita juga bisa menggunakan operator (+) yang berfungsi untuk menggabungkan dua teks yang terpisah menjadi satu         buah teks. ini juga bisa dikenal sebagai string concatenation seperti diatas akan menggabungkan string apa adanya. javascript juga mendukung string                     interpolation, sederhananya, kita bisa memasukkan variabel ke dalam sebuah string template. kita bisa menggunakan backsticks (`) untuk mendefinisikan string           template.
...
Belajar Dasar Pemrograman Javascript
   ...
   * Pengenalan Struktur Data
     ...
     - Map
     - Set
   * Object berfungsi untuk menyimpan nilai dari beragam tipe data dan membentuk data yang lebih kompleks, untuk menetapkan objek pada sebuah variabel kita gunakan          tanda kurung kurawal.
...
Belajar Dasar Pemrograman Javascript
    ...
    * Pure Function adalah konsep dari pembuatan fungsi yang mengharuskan fungsi untuk tidak bergantung terhadap nilai yang berada diluar fungsi atau parameternya.           Sehingga mau seperti apa pun keadaannya, fungsi yang dibuat selalu menghasilkan sesuatu yang sama, terkecuali bila fungsi tersebut diberikan nilai parameter           yang berbeda. Pure Function juga dilarang keras untuk mengubah nilai yang berada di luar baik secara sengaja atau tidak sengaja. Pure Function tidak boleh             menimbulkan efek samping (no side effect) ketika digunakan.
      - Konsep dari Pure Function ada 3 : mengembalikan nilai yang sama bila inputannya (nilai parameter) sama. Hanya bergantung pada argumen yang diberikan. Tidak             menimbulkan efek samping.
    * Immutability adalah sebuah objek tidak boleh diubah setelah objek tersebut dibuat. Kontras dengan mutable yang artinya objek boleh diubah setelah objek tersebut       dibuat.
    * Rekursif merupakan teknik pada sebuah function yang memanggil dirinya sendiri. Dengan rekursif dapat membuat dan mengolah data structures seperti Tree dan Array.
    * High Order Function merupakan bagian dari konsep Functional Programming, memiliki fungsi yang dapat menerima fungsi lainnya pada argumen; mengembalikan sebuah         fungsi; atau bahkan keduannya.
      - Teknik Higher Order Function biasanya digunakan untuk : mengabstraksi atau mengisolasi sebuah aksi, event, atau menangani alur asynchronous menggunakan                 callback, promise, dan lainnya. Membuat utilites yang dapat digunakan di berbagai tipe data. Membuat teknik currying atau function composition.
      - Array map() merupakan salah satu contoh Higher-Order Function yang ada di JavaScript. Karena dalam pemggunaannya, ia menerima satu buah argumen yang merupakan         sebuah function.
Senin 27 Maret 2023
Belajar Dasar Pemrograman Javascript
    * Reusable Function
      - Array Map, fungsi array.map() merupakan fungsi bawaan dari array yang sangat berguna, fungsi ini dapat dipanggil dari sebuah data bertipe array dan menerima           satu buah callback function.
      - Array Filter, fungsi array.filter() merupakan fungsi bawaan dari data yang bertipe array di JavaScript, fungsi ini sangat berguna untuk melakukan proses               penyaringan (filtering) terhadap nilai array yang ada.
      - Array Reduce, array.reduce() adalah fungsi bawaan dari data yang bertipe array yang digunakan untuk mengeksekusi fungsi reducer pada setiap elemen array dan           hanya mengembalikan output satu nilai saja.
      - Array Some, array.some() merupakan fungsi bawaan dari array yang cukup sering digunakan dan fungsi ini akan menghasilkan nilai boolean.
      - Array Find, array.find() sesuai namanya digunakan untuk mencari apakah di dalam deretan nilai terdapat nilai yang sesuai dengan kriteria yang kita definisikan         pada callback function. Yang membedakan array.find() dengan array.some(), find akan menghasilkan satu nilai dari elemen yang pertama kali ditemukan berdasarkan         kriteria tertentu dan akan menghasilkan nilai undefine bila tidak ada kriteria yang terpenuhi pada item array.
    * Array Sort, array.sort() merupakan fungsi bawaan dari array yang berguna untuk melakukan pengurutan nilai dari sebuah deretan nilai. Secara default, fungsi sort       akan mengubah semua nilai dalam deretan menjadi bentuk string dan mengurutkannya secara ascending.
    * Array Every, array.every() merupakan fungsi bawaan dari array yang digunakan untuk mengecheck apakah semua nilai dari sebuah array sesuai dengan kriteria yang         didefinisikan. Kembalian dari array.every() adalah nilai Boolean.
    * Array forEach, Array forEach merupakan fungsi bawaan dari array yang berfungsi untuk memanggil fungsi callback pada setiap iterasi index array dan fungsi ini           tidak mengembalikan nilai apa pun.
